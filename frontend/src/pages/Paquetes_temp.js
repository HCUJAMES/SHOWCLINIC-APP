...
const Paquetes = () => {
  ...
  const abrirModalEditar = (paquete) => {
    ...
    setTratamientosSeleccionados(paquete.tratamientos || []);
    setPrecioRegular(paquete.precio_regular || 0);
    setPrecioPaquete(paquete.precio_paquete || 0);
  ...
  const agregarTratamiento = (tratamiento) => {
    const yaExiste = tratamientosSeleccionados.some(t => t.id === tratamiento.id);
    if (yaExiste) {
      showToast({ severity: "warning", message: "Este tratamiento ya está agregado" });
      return;
    }
    setTratamientosSeleccionados([...tratamientosSeleccionados, { ...tratamiento, sesiones_tratamiento: 1 }]);
  };

  const eliminarTratamiento = (tratamientoId) => {
    setTratamientosSeleccionados(tratamientosSeleccionados.filter(t => t.id !== tratamientoId));
  };

  const actualizarSesionesTratamiento = (tratamientoId, sesiones) => {
    setTratamientosSeleccionados(tratamientosSeleccionados.map(t => 
      t.id === tratamientoId ? { ...t, sesiones_tratamiento: parseInt(sesiones) || 1 } : t
    ));
  };
                  ...
                  <TableRow key={paquete.id} hover>
                    ...
                    <TableCell>
                      <Chip
                        ...
                        size="small"
                        sx={{
                          backgroundColor: paquete.estado === "activo" ? "#e8f5e9" : "#ffebee",
                          color: paquete.estado === "activo" ? "#2e7d32" : "#c62828",
                        }}
                      />
                    </TableCell>
                    <TableCell>
              ...
              <Grid item xs={12} md={6} sx={{ p: 4, backgroundColor: "#fafafa" }}>
                ...
                <Box sx={{ mt: 3, p: 2, backgroundColor: "#e3f2fd", borderRadius: 2 }}>
                  <Typography variant="body2" color="primary">
                    💡 <strong>Tip:</strong> Los paquetes con fechas de vigencia limitadas generan mayor urgencia de compra.
                  </Typography>
                </Box>
              </Grid>
            </Grid>
